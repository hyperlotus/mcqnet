...
```bash
cmake -S . -B build -DMCQNET_BUILD_BENCHMARKS=ON
cmake -S . -B build -DMCQNET_ENABLE_CLANG_TIDY=ON
```
...
- `project_context.md`: 静态背景、架构、公开接口和数据流
- `project_progress.md`: 当前进度、已完成能力、近期整理和下一阶段重点
- `linux_socket_handoff.md`: Linux socket 层交接说明
...
1. 填完 `linux_socket_api.h` 里的真实 syscall 语义
